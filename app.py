...
st.markdown("*MDN v10 — Agent-based autoregressive simulation over 52 weeks*")
st.markdown("---")
...
with col_left:
    ...
    st.subheader("🛡️ Baseline resilience (z-score)")
    res_vals = [st.slider(lab, -3.5, 3.5, 0.0, 0.1, key=f'res_{i}')
    ...
    ind_z = float(np.mean(res_vals[:4]))
    fam_z = float(res_vals[4])
    soc_z = float(res_vals[5])
    ...
    for col, label, grp, color in [(c1,'IND',grp_ind,col_ind),(c2,'FAM',grp_fam,col_fam),(c3,'SOC',grp_soc,col_soc)]:
        col.markdown(f"<div style='text-align:center;background:{color};color:white;padding:8px;border-radius:8px'><b>{label}</b><br>{grp}</div>", unsafe_allow_html=True)
...
with col_right:
    ...
    fig, axes = plt.subplots(2, 1, figsize=(11, 8), gridspec_kw={'height_ratios': [3.5, 1]})
    ax = axes[0]

    for i in range(N_OUT):
        dm = all_traj[:,:,i].mean(axis=0)
        ds = all_traj[:,:,i].std(axis=0)
        ax.plot(weeks, dm, color=COLORS[i], lw=1.8, label=LABELS_SHORT[i], alpha=0.9)
        ax.fill_between(weeks, dm-ds, dm+ds, color=COLORS[i], alpha=0.12)

    ax.plot(weeks, comp_traj, color='black', lw=2.5, linestyle='--', label='Composite', zorder=5)
    ax.axhline(0,      color='gray',      lw=0.8, linestyle=':')
    ax.axhline(THETA,  color='red',       lw=1.2, linestyle='--', alpha=0.7, label=f'θ={THETA}')
    ax.axhline(-THETA, color='steelblue', lw=0.8, linestyle='--', alpha=0.3)
    ax.set_xlim(0, T_STEPS); ax.set_ylim(-3.5, 3.5)
    ax.set_ylabel('Craving z-score', fontsize=12)
    ax.set_title(f'Simulated craving trajectory — 52 weeks  (baseline composite: {comp_base:+.2f})\n'
                 f'Resilience — IND: {grp_ind}  |  FAM: {grp_fam}  |  SOC: {grp_soc}', fontsize=11)
    ...
    ax2.set_xlim(0, T_STEPS); ax2.set_yticks([])
    ax2.set_xlabel('Week', fontsize=11); ax2.set_ylabel('Latent state', fontsize=9)
    ax2.legend(handles=[mpatches.Patch(color=sc[s], label=sl[s]) for s in [0,1,2]],
    ...
    m1.metric("Composite mean (52w)", f"{comp_traj.mean():+.2f}")
