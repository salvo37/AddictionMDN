...
with col_left:
    ...
    st.subheader("🛡️ Baseline resilience — domains (z-score)")
    ind_z = st.slider("IND — Individual domain", -3.5, 3.5, 0.0, 0.1, key='res_ind')
    fam_z = st.slider("FAM — Family domain",     -3.5, 3.5, 0.0, 0.1, key='res_fam')
    soc_z = st.slider("SOC — Social domain",     -3.5, 3.5, 0.0, 0.1, key='res_soc')
    ...
    for col, label, grp, color in [
        (c1, 'IND', grp_ind, col_ind),
        (c2, 'FAM', grp_fam, col_fam),
        (c3, 'SOC', grp_soc, col_soc)
    ]:
        col.markdown(
            f"<div style='text-align:center;background:{color};color:white;"
            f"padding:8px;border-radius:8px'><b>{label}</b><br>{grp}</div>",
...
with col_right:
    ...
    ax.plot(weeks, comp_traj, color='black', lw=2.5, linestyle='--',
    ...
    ax.set_title(
        f'Simulated craving trajectory — 52 weeks  '
        f'(baseline composite: {comp_base:+.2f})\n'
        f'Resilience — IND: {grp_ind}  |  FAM: {grp_fam}  |  SOC: {grp_soc}',
        fontsize=11
    )
